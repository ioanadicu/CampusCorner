function viewEvent(description) {
    ...
    fetch(`/event`)
        ...
        .then(eventHtml => {
            ...
            const mainPopup = document.getElementById("popupPanel");
            mainPopup.style.display = 'flex';
            const popupPanel = document.getElementById("event-container");

            //popupPanel.style.display = "block";
            overlay.style.display = "block";

            if (popupPanel.style.display === 'none') {
                popupPanel.style.display = 'block'; // Show right column
              } 
        });
